...
SELECT MatchTime, Team, COUNT(*) AS Match_Count,
				  ...
				  SUM(CASE WHEN Team = HomeTeam OR AwayTeam AND FTHome = FTAway THEN 1 ELSE 0 END) AS Draws
FROM Matches
...
WHERE Division = 'E0'
				AND (HomeTeam OR AwayTeam = 'Arsenal')
				AND MatchDate BETWEEN '2019-08-01' AND '2025-05-31'
GROUP BY MatchTime, Team
ORDER BY Team;
